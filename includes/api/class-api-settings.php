 ...
 * @subpackage WP_EXT_RULE_PRICING/WP_EXT_RULE_PRICING_Api_Settings
 */
 ...
 *
 * @see WP_EXT_RULE_PRICING_Api
 */
if ( ! class_exists( 'WP_EXT_RULE_PRICING_Api_Settings' ) ) {

	/**
	 * WP_EXT_RULE_PRICING_Api_Settings
	 *
	 * @see WP_REST_Settings_Controller
	 * @package WP_EXT_RULE_PRICING
	 * @since 1.0.0
	 */
	class WP_EXT_RULE_PRICING_Api_Settings extends WP_EXT_RULE_PRICING_Api {
		 ...
		 */
		public function run() {
			$this->type      = 'wp_ext_rule_pricing_api_settings';
		...
		public function get_item( $request ) {
			...
			$saved_options = wp_ext_rule_pricing_get_options();
		...
		public function update_item( $request ) {
			...
			if ( is_wp_error( rest_validate_value_from_schema( $params, $schema ) ) ) {
				return new WP_Error(
					...
					sprintf( __( 'The %s property has an invalid stored value, and cannot be updated to null.', 'wp-ext-rule-pricing' ), WP_EXT_RULE_PRICING_OPTION_NAME ),
					array( 'status' => 500 )
				);
			}

			$sanitized_options = $this->prepare_value( $params, $schema );
			wp_ext_rule_pricing_update_options( $sanitized_options );
		...
		protected function get_registered_schema() {
			...
			$schema = wp_ext_rule_pricing_admin()->get_settings_schema();
...
/**
 * Return instance of  WP_EXT_RULE_PRICING_Api_Settings class
 *
 * @since 1.0.0
 *
 * @return WP_EXT_RULE_PRICING_Api_Settings
 */
function wp_ext_rule_pricing_api_settings() { //phpcs:ignore
	return WP_EXT_RULE_PRICING_Api_Settings::get_instance();
}
wp_ext_rule_pricing_api_settings()->run();
