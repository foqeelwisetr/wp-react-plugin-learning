...
var import_canvas_utils = require("../canvas-utils.cjs");
var import_feature_detection = require("../feature-detection.cjs");
...
function prepareItem(id) {
  return async ({ select, dispatch }) => {
    ...
    const isHeic = import_constants.HEIC_MIME_TYPES.includes(file.type);
    if (isImage && isVipsSupported) {
      ...
      try {
        heicJpeg = await (0, import_canvas_utils.canvasConvertToJpeg)(
          file,
    ...
    let updates = {};
    if (isHeic && heicJpeg) {
      const vipsAvailable = (0, import_feature_detection.isClientSideMediaSupported)();
      updates = {
        file: heicJpeg,
        sourceFile: heicJpeg,
  ...
  return async ({ select, dispatch }) => {
    ...
    mediaSideload({
      ...
      onSuccess: (subSize) => {
        if (item.parentId) {
          dispatch({
            type: import_types.Type.AccumulateSubSize,
            id: item.parentId,
            subSize
          });
        }
...
function generateThumbnails(id) {
  return async ({ select, dispatch }) => {
    ...
    if (!item.parentId && attachment.missing_image_sizes && attachment.missing_image_sizes.length > 0) {
      ...
      const file = attachment.filename ? (0, import_utils.renameFile)(thumbnailSource, attachment.filename) : thumbnailSource;
      ...
      }
      {
        ...
        if (bigImageSizeThreshold && attachment.id) {
          ...
          bitmap.close();
          if (needsScaling) {
            const sourceForScaled = attachment.filename ? (0, import_utils.renameFile)(thumbnailSource, attachment.filename) : thumbnailSource;
