...
function getSaveElement(blockTypeOrName, attributes, innerBlocks = []) {
  ...
  if (element !== null && typeof element === "object" && (0, import_hooks.hasFilter)("blocks.getSaveContent.extraProps") && !((blockType.apiVersion ?? 0) > 1)) {
    ...
    if (!(0, import_is_shallow_equal.isShallowEqual)(props, element.props)) {
      element = (0, import_element.cloneElement)(
        element,
        props
      );
    }
...
function getSaveContent(blockTypeOrName, attributes, innerBlocks) {
  const blockType = (0, import_utils.normalizeBlockType)(
    blockTypeOrName
  );
  if (!blockType) {
    return "";
...
var __unstableSerializeAndClean = /* @__PURE__ */ (() => {
  ...
  return (blocks) => {
    ...
    if (effectiveBlocks.length === 1 && (0, import_utils.isUnmodifiedDefaultBlock)(effectiveBlocks[0])) {
      effectiveBlocks = [];
    }
    let content = serialize(effectiveBlocks);
    if (effectiveBlocks.length === 1 && effectiveBlocks[0].name === (0, import_registration.getFreeformContentHandlerName)() && effectiveBlocks[0].name === "core/freeform") {
