...
var Delta = class _Delta {
  ...
  diffWithCursor(other, cursorAfterChange) {
    ...
    if (maxStringLength > STRING_TOO_LARGE_THRESHOLD) {
      const diffResult = normalizeChangeCounts(
        (0, import_diff.diffLines)(strings[0], strings[1])
      );
      const thisIterLarge = new import_OpIterator.default(this.ops);
      const otherIterLarge = new import_OpIterator.default(other.ops);
