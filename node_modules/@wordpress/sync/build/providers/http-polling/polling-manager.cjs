...
function handlePageHide() {
  ...
  for (let i = 0; i < rooms.length; i += import_config.MAX_ROOMS_PER_REQUEST) {
    (0, import_utils.postSyncUpdateNonBlocking)({
      rooms: rooms.slice(i, i + import_config.MAX_ROOMS_PER_REQUEST)
    });
  }
}
...
function selectRoomsForRequest() {
  ...
  if (allRooms.length <= import_config.MAX_ROOMS_PER_REQUEST) {
    return allRooms;
  }
  const primaryRoom = allRooms.find((state) => state.isPrimaryRoom);
  const overflowRooms = allRooms.filter((state) => state !== primaryRoom);
  const overflowSlotsPerRequest = import_config.MAX_ROOMS_PER_REQUEST - (primaryRoom ? 1 : 0);
  const { window: overflowSlice, nextOffset } = (0, import_utils.rotateWindow)(
    overflowRooms,
    roomOverflowOffset,
  ...
  async function start() {
    ...
    try {
      ...
      if (isForbiddenError(error)) {
        ...
        consecutiveFailures++;
        const retrySchedule = hasCollaborators ? import_config.ERROR_RETRY_DELAYS_WITH_COLLABORATORS_MS : import_config.ERROR_RETRY_DELAYS_SOLO_MS;
        if (consecutiveFailures <= retrySchedule.length) {
          pollInterval = retrySchedule[consecutiveFailures - 1];
        } else {
          pollInterval = import_config.DISCONNECT_DIALOG_RETRY_MS;
        }
        if (isManualRetry) {
          pollInterval = import_config.MANUAL_RETRY_INTERVAL_MS;
          isManualRetry = false;
        }
